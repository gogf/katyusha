...
func Test_Ctx_Basic(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		ctx := krpc.Ctx.IngoingToOutgoing(ctx)
		m1 := krpc.Ctx.IngoingMap(ctx)
		t.Assert(m1.Get("k1"), "v1")
		t.Assert(m1.Get("k2"), "v2")
		m2 := krpc.Ctx.OutgoingMap(ctx)
	...
	gtest.C(t, func(t *gtest.T) {
		ctx := krpc.Ctx.IngoingToOutgoing(ctx, "k1")
		m1 := krpc.Ctx.IngoingMap(ctx)
		t.Assert(m1.Get("k1"), "v1")
		t.Assert(m1.Get("k2"), "v2")
		m2 := krpc.Ctx.OutgoingMap(ctx)
	...
	gtest.C(t, func(t *gtest.T) {
		ctx := krpc.Ctx.NewIngoing(ctx)
		ctx = krpc.Ctx.SetIngoing(ctx, g.Map{"k1": "v1"})
		ctx = krpc.Ctx.SetIngoing(ctx, g.Map{"k2": "v2"})
