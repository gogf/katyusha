...
func TestRegistryGet(t *testing.T) {
	r := NewRegistry()
	r.Register("foo", NewCounter())
	if count := r.Get("foo").(ICounter).Count(); 0 != count {
		t.Fatal(count)
	}
	r.Get("foo").(ICounter).Inc(1)
	if count := r.Get("foo").(ICounter).Count(); 1 != count {
		t.Fatal(count)
	}
}
